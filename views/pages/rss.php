...
foreach($questions as $q) {
	...
	if($q->item_type == 'post') {
		$item_type = "Post: ";
		$q_link = $url_mapper['posts/view'].rawurlencode($url_type);
	} else {
		$item_type = "Question: ";
	...
	$builder .="<item>
			<title>{$item_type}: {$q->title}</title>
			  <link>{$q_link}</link>
			  <guid isPermaLink='true'>{$q_link}</guid>
			  <description>"; 
			  $builder.=strip_tags($q->content, '<a><img><b><i><p><u><ol><ul><li><iframe>'); 
			  $builder .="</description>
			  <pubDate>"; 
...
if(!empty($answers)) {
	...
	if($q->item_type == 'post') {
		$q_link = $url_mapper['posts/view'].rawurlencode($url_type).'#answer-'.$a->id;
	} else {
