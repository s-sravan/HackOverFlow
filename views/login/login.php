...
if( isset( $_GET["provider"] ) && isset($settings['social']) && $settings['social'] == '1' ){
	$provider_name = $db->escape_value($_GET["provider"]);

	try {
		include_once(UPLOAD_PATH . '/hybridauth/config.php');
		include_once(UPLOAD_PATH . '/hybridauth/autoload.php');
		$config['callback'] = WEB_LINK. 'login/?provider=' . $provider_name;
			...
			$saltedhash = $found_user->password;
				...
				if ($phpass->CheckPassword($password, $saltedhash)) {
					...
					if(isset($_POST['next']) && $_POST['next'] != '' ) {
						$next = $db->escape_value(strip_tags($_POST['next']));
					} else {
						$next = $url_mapper['index/'];
					}
					redirect_to($next);
				} else {
      ...
      <div class="site-wrapper-inner">
        ...
        <div class="cover-container">
			...
			<div class="inner cover " >
				<div class="loginbox">
				<h3 style="color:#b92b27;font-size:45px;" class="login-brand"><center><?php 
						...
						<div class="col-md-6 ">
							...
							<form method="POST" action="<?php echo $url_mapper['login/']; ?>" >
									<div class="form-group">
										<label for="email" class="col-form-label"><?php echo $lang['login-register-email']; ?></label>
										<input type="email" class="form-control " id="email" name="email" placeholder="" value="" required>
										<input type="checkbox" name="remember-me"> <?php echo $lang['login-remember']; ?>
									</div>
									<div class="form-group">
										<label for="password" class="col-form-label"><?php echo $lang['login-register-pass']; ?></label>
										<input type="password" class="form-control " id="password" name="password" placeholder="" value="" required>
										<a href="javascript:void(0);" id="forgot-password" style="color:grey"><?php echo $lang['login-forgot_pass']; ?></a>
									</div>
							...
							<div id="forgot-password-block" style="display:none">
								...
								<form method="POST" action="<?php echo $url_mapper['login/']; ?>" >
									<div class="form-group">
										<label for="forgot-email" class="col-form-label"><?php echo $lang['login-register-email']; ?></label>
										<input type="email" class="form-control " id="forgot-email" name="forgot-email" placeholder="" value="" required>
									</div>
									
									<a href="javascript:void(0);" id="back-to-login" class="btn btn-primary"><?php echo $lang['btn-back']; ?></a>&nbsp;
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal"><?php echo $lang['btn-close']; ?></button>
      </div>
    </div>
  </div>
</div>
