...
function ads($placement = 'between_questions') {
	$ads = AdManager::get_everything(" AND location LIKE '%{$placement}%' ");
	$return = array();
	if($ads) {
		foreach($ads as $a) {
			$created = strtotime($a->created_at);
			$expiry = strtotime("+{$a->expiry}" , $created );
			
			if($a->expiry == '' || $a->expiry == '0' || $expiry > time() ) {
				$return[] = $a;
			}
		}
	}
