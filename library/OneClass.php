...
class OneClass {
	...
	public static function check_existance($column="" , $value="" , $extra="") {
	global $db;
	$column = $db->escape_value($column);
	$value = $db->escape_value($value);
	$extra = $db->escape_value($extra);
	
	$sql = "SELECT id FROM  " .  DBTP. static::$table_name ;
	$sql .= " WHERE {$column} = '{$value}' {$extra} ";
	$sql .= "LIMIT 1" ;
	...
	protected function clean_attributes() {
		...
		$q = $db->query('DESCRIBE '.DBTP. static::$table_name);
		$structure = array();
		while($row = $db->fetch_array($q)) {
			$type = explode('(' , $row['Type']);
			$structure[$row['Field']] = $type[0];
		}
		foreach ($this->attributes() as $key => $value ) {
			if($value == '') {
				if($structure[$key] == 'int' || $structure[$key] == 'tinyint') {	//STRICT_TRANS_TABLES fix
					$clean_attributes[$key] = 0;
				} elseif($structure[$key] == 'datetime') {
					$clean_attributes[$key] = '0000-00-00 00:00:00';
				} elseif($structure[$key] == 'date') {
					$clean_attributes[$key] = '0000-00-00';
				} else {
