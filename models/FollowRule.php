...
class FollowRule Extends OneClass {
	...
	public static function get_subscriptions($obj_type='question' , $id="" , $col = 'user_id' , $except ='' , $string="") {
		global $db;
		$not_in = '';
		if(is_array($except)) {
			$id_arr = implode(',' , $except);
			$not_in = " AND obj_id NOT IN({$id_arr}) ";
		}
		return self::preform_sql("SELECT * FROM ".DBTP ."follows_rules WHERE obj_type = '{$obj_type}' AND {$col} = '{$id}' {$not_in} ORDER BY follow_date DESC " . $string );
	}
