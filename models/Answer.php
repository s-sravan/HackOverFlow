...
class Answer Extends OneClass {
	...
	public static $db_fields = array( "id" , "user_id" , "q_id" , "a_id", "content", "likes", "dislikes", "anonymous", "created_at" , "updated_at" , "published");
	...
	global $db;
	$result_array =  static::preform_sql("SELECT * FROM " . DBTP . self::$table_name . " WHERE q_id = " . $q_id . " AND a_id = 0 AND anonymous != 2 AND published = 1 ORDER BY likes DESC LIMIT 1");
	return !empty($result_array) ? array_shift($result_array) : false;
	}

	public static function get_pending($string = "") {
		global $db;
		return self::preform_sql("SELECT * FROM " . DBTP . self::$table_name . " WHERE published = 0 {$string} AND a_id = 0 ORDER BY created_at ASC " . $string );
	}
	public static function count_pending($string = "") {
		//get feed ...
		global $db;
		$result = $db->query("SELECT COUNT(id) FROM " . DBTP . self::$table_name . " WHERE published = 0 {$string} AND a_id = 0 ORDER BY created_at ASC " . $string );
		return mysqli_result($result, 0);
	}
	
	public static function get_answers_for($q_id , $string = "") {
		global $db;
		return self::preform_sql("SELECT * FROM " . DBTP . self::$table_name . " WHERE q_id = " . $q_id . " AND a_id = 0 ORDER BY likes DESC " . $string);
	}
	
	public static function get_sub_answers_for($a_id , $string = "") {
		global $db;
		return self::preform_sql("SELECT * FROM " . DBTP . self::$table_name . " WHERE a_id = " . $a_id . " ORDER BY likes DESC " . $string);
	}
